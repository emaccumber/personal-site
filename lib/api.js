...
export function getAllPhotoAlbums() {
  try {
    // First try to load from the new 'photographs' directory
    try {
      const albums = require('../_content/photographs/albums.json').albums;
      return albums;
    } catch (e) {
      // If that fails, fall back to the old 'photography' directory for backwards compatibility
      console.log('Falling back to photography directory', e);
...
export function getPhotoAlbumBySlug(slug) {
  try {
    ...
    try {
      ...
      // If that fails, fall back to the old 'photography' directory for backwards compatibility
      console.log(`Falling back to photography directory for album: ${slug}`, e);
...
export function getMovingImageAlbumBySlug(slug) {
  try {
    ...
    if (slug === '16mm-project') {
      return {
        ...
        clips: [
          ...
          {
            id: 3,
            src: "/videos/films/16mm-project/clip3.mp4",
    ...
    // const album = require(`../_content/films/${slug}.json`);
    // return album;
